...
impl super::ProxyFetcher for ProxyFetcher {
    async fn fetch(&self, sender: ProxySender) -> Result<()> {
        let _ = inner_fetch("?stype=1", &sender).await?;

        inner_fetch("?stype=2", &sender).await
    }
}

async fn inner_fetch(query: &str, sender: &ProxySender) -> Result<()> {
    let html = http::get(format!("http://www.ip3366.net/free/{query}"))
        .await?
        .xpath()
        .await?;
